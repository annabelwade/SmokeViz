...
def get_prev_iou_by_density(iou_dict):
    try:
        high_iou = iou_dict['high']['prev_int']/iou_dict['high']['prev_union']
    except ZeroDivisionError:
        high_iou = -1 #float('nan') # np.nan
    ...
    try:
        iou = (iou_dict['high']['prev_int'] + iou_dict['medium']['prev_int'] + iou_dict['low']['prev_int'])/(iou_dict['high']['prev_union'] + iou_dict['medium']['prev_union'] + iou_dict['low']['prev_union']) # current overall IoU calculation
    except ZeroDivisionError:
        iou = -1# float('nan')
